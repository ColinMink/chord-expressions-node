...
Chord.prototype.contains = function(arr){
    //The chord always contains nothing
    let flag = true;
    if(!arr){
        return true;
    }
    arr.forEach(note => {
        if(this.notes.find((cNote) => cNote.name === note) === undefined){
            flag = false;
        }
    })
    return flag;
...
Chord.prototype.hasAtLeastOne = function(arr){
    //The chord always contains nothing
    let flag = false;
    if(!arr){
        return true;
