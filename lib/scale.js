...
const Modes = [
 ...
 
    {
        scales: {
            ...
            "Lydian Dominant": ["1","b2","3","#4","5","6","b7"],
...
module.exports = {Scale, Modes, fromSimple};

function Scale(rootNote,groupID,groupName, scale){
    if(Modes[groupID] !== undefined && Modes[groupID].scales[scale] !== undefined) {
        this.groupID = parseInt(groupID);
        this.groupName = groupName;
        this.scaleName = scale;
        this.intervalArray = Modes[groupID].scales[scale];
    } 
    else {
        throw("Invalid mode or scale name provdied: " + groupID + " | " + scale );
    }
...
/* Dictate a scale object instead of having one generated from template objects
   Useful for pulling scales from the database, once DB translated into required paramaters (specifically the noteNameList)
   Note that this does no calculation for you and requires that you send in data you know to be correct, reliable, where all parameters agree with eachother */
// rootStr: e.g. "E"
// nameStr: e.g. "Dorian"
// noteNameList: e.g. ["E", "F#", "G", "A", "B", "C#", "D"]
function fromSimple(rootStr, nameStr , noteNameList) {
    if (typeof nameStr !== "string") {throw new Error("nameStr must be a string but is instead: " + (typeof nameStr));}

    const scale = {
        //rootNote: Note.fromName(rootStr),
        root: rootStr, // app expects string
        name: rootStr + " " + nameStr,
        //notes: noteNameList.map((noteName) => Note.fromName(noteName)),
        notes : noteNameList.map((noteName) => {
            const note = {...Note.fromName(noteName)}; // clone obj just to be safe and cool
            note.label = note.name; //app wants label not name. TODO: should I change the app to use name? we had to map iterate here anyway. see line above
            delete note.name;
            return note;
        }),
    };
...
console.log(fromSimple("E", "Dorian", ["E", "F#", "G"]));